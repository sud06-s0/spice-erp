...
export default function ExportOrderFormModal({ customers, finishedProducts, onClose, onCreated, onCustomerAdded }) {
  ...
  const handleAddCustomer = async () => {
    ...
    if (!newCustomerName.trim()) {
      setAddCustomerError('Type a customer name first')
      return
    }
    ...
    <Modal title="Create Export Order" onClose={onClose} width="max-w-2xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="mt-2 rounded-md border border-border bg-surface-muted px-3 py-2.5">
            <p className="text-xs text-ink-muted mb-2">Or add a new customer</p>
            <div className="flex gap-2">
              <input
                type="text"
                value={newCustomerName}
                onChange={(e) => setNewCustomerName(e.target.value)}
                placeholder="Name (e.g. Hanseatic Spice GmbH)"
                className="flex-1 rounded-md border border-border bg-bg px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <input
                type="text"
                ...
                placeholder="Country"
                className="w-28 rounded-md border border-border bg-bg px-3 py-2 text-sm text-ink focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <button
                type="button"
                onClick={handleAddCustomer}
                disabled={addingCustomer}
                className="shrink-0 px-4 py-2 text-sm font-medium rounded-md bg-primary text-white hover:bg-primary-hover disabled:opacity-60 transition-colors"
              >
                {addingCustomer ? 'Adding…' : 'Add'}
              </button>
            </div>
            {addCustomerError && (
              <p className="text-xs text-danger mt-1.5">{addCustomerError}</p>
            )}
          </div>
        </div>
